...
wCam, hCam = 1280, 720 #width of camera, height of camera
offset = 60 #특정 범위 내 검지가 들어오면 print screen을 하기 위해 offset을 지정
...
drag_flag, no_dup_drag, t1_flag, thread_flag = False, True, True, True
...
def Print_screen(): #print screen 함수
    global thread_flag #전역변수 사용
    img_print = pyautogui.press('printscreen') #printscreen key 누름
    img1 = ImageGrab.grab(img_print) #key 누름과 동시에 캡처된 이미지
    img1.save('./screenshot.png') #이미지 저장
    img1.show() #이미지 화면으로 띄움
    thread_flag = True
...
while True:
    ...
    rect1 = cv2.rectangle(imgNew, (wCam-offset, 0), (wCam, hCam), color=0, thickness=0, lineType=None, shift=None)
    ...
    if hands:  # 손 감지되었을때 시작
        ...
        if fingers == [1, 1, 1, 1, 1]:  # 손가락을 모두 폈을때 마우스 동작 하지 않도록 함.
            ...
            x1, y1 = lmList[8] #검지 끝 인식

            if (wCam-offset < x1 < wCam) and thread_flag: #x1의 좌표가 해당 범위에 들어오면 print screen 처리
                t4 = Thread(target=Print_screen)
                thread_flag = False
                t4.start()
