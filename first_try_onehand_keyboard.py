...
input_status = True
global temp_arr, thread_flag
temp_arr = []
thread_flag = True

def Input_finger(fingers, st=0):
    if fingers[0] == 0:
...
def choice_word(fingers, word_arr):
    try:
        if fingers[0] == 0: return True, word_arr[0]
        elif fingers[1] == 0: return True, word_arr[1]
        elif fingers[2] == 0: return True, word_arr[2]
        elif fingers[3] == 0: return True, word_arr[3]
    except: return False, ''
        ...
        if fingers != [1, 1, 1, 1, 1]:
            if input_status:
                if math.sin((math.pi / 2) - (math.pi / 6)) < (abs(y2 - y1) / distance):
                    temp_arr = Input_finger(fingers)
                    input_status = False
                    input_word, big_word_flag = judge_space_toggle(temp_arr)
                elif 0 < (abs(y2 - y1) / distance) < math.sin(math.pi / 6):
                    temp_arr = Input_finger(fingers, st=5)
                    input_status = False
                    input_word, big_word_flag = judge_space_toggle(temp_arr, fl=True)

            else: input_status, input_word = choice_word(fingers, temp_arr)
